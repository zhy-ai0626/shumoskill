...
**Tornado 图有一条硬规矩：必须按影响幅度排序，不排序等于没画。** 现成实现在
`figures/starter.py` 第 4 段（`--only 4`）与 `figures/gallery.py` 的 `06`，
两者都已排序、都带上下界图例。箱线/区间图见 starter 第 1 段 (b)；
**离群点保留并标出，不要先删掉再画**——删掉离群点的箱线图看不出风险，
而 Stage 6 交付的就是风险。
